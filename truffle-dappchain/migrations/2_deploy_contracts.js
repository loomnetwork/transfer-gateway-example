...
module.exports = (deployer, network, accounts) => {
  ...
  const gatewayAddress = process.env.IS_DOCKER ? process.env.GATEWAY_DAPPCHAIN_ADDRESS : readFileSync('/data/gateway_dappchain_address', 'utf-8') 
  ...
  deployer.deploy(CryptoCardsDappChain, gatewayAddress).then(async () => {
    ...
    writeFileSync('/dappchain/crypto_cards_dappchain_address', cryptoCardsDAppChainInstance.address)
  })
  ...
  deployer.deploy(GameTokenDappChain, gatewayAddress).then(async () => {
    ...
    writeFileSync('/dappchain/game_token_dappchain_address', GameTokenDappChainInstance.address)
  })
}
